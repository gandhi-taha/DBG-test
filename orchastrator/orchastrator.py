...
import vertexai
from orchastrator.imagesearch import get_gcs_location_for_query
...
with open(
    os.path.join(os.path.dirname(__file__), "router_prompt_template.txt"), "r"
) as f:
...
def generate_router_chain(llm):
    ...
    for e in prompt_factory.prompt_infos:
        ...
        if "agent" in e:
            ...
            destination_chains[name] = p_template | VertexAI(
                model_name="gemini-pro", temperature=1
            )

    router_chain = (
        router_prompt.partial(routes_str=format_routes(agent_descriptions))
        | llm
        | StrOutputParser()
    )
    ...
    } | RunnableLambda(lambda x: destination_chains[x["topic"]])
...
class Orchastrator:
    ...
    def image_search(self, query):
        with open(
            os.path.join(
                os.path.dirname(__file__), "image_retrieval_promp_template.txt"
            ),
            "r",
        ) as f:
            image_retrieval_prompt = PromptTemplate.from_template(f.read())

        image_chain = image_retrieval_prompt | self.llm
        response = image_chain.invoke({"question": query["question"]})

        if response == "True":
            return get_gcs_location_for_query(query["question"])
        else:
            return ""
